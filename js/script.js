/* Authors: Jonathan Carson, Shane Eckel */
$(function(){

	// Get/set height for #container and #nav (sets up sticky nav for fancy animations 'n at)
	var mainHeight = $('#main').height();
	var navHeight = $('#nav').height();
	$('#container').css('height', mainHeight + navHeight);

	// Initialize sticky nav
	$('#nav').stickyfloat({ duration: 500 });
	
	// Sub-Nav rollover
	$('#menu-sub-nav-wrap').hover(
		// Mouse over
		function() {
			$('#menu-sub-nav').css('display', 'block');
			$('#menu-sub-nav').stop().animate({opacity: 1}, 100);
		},
		
		// Mouse out
		function() {
			$('#menu-sub-nav').stop().animate({opacity: 0}, 100);
			$('#menu-sub-nav').css('display', 'none');
		}
	);	
	
	// HP Location Rollover
	$('.hp-location-link').hover(
		//Mouse over
		function() {
			$(this).find('div').stop().animate({opacity: 1}, 250 );
		},
		
		//Mouse out
		function() {
			$(this).find('div').stop().animate({opacity : 0}, 250 );
		}
	);	
	...
	// Featured menu image rollovers (3 column)
	$('.featured-photo-triple-link').hover(
		//Mouse over
		function() {
			$(this).find('div').stop().animate({opacity: 1}, 250 );
		},
	...
	// Featured menu image rollovers (2 column)
	$('.featured-photo-double-link').hover(
		//Mouse over
		function() {
			$(this).find('div').stop().animate({opacity: 1}, 250 );
		},
	...
	// Footer-shmooter
	// Clicky shtuff
	$('#footer-off-home').click(
		function() {
		
			var id = $(this).attr('id');
		
			if (id == 'footer-off-home') {
				$(this).stop().animate({bottom: '256px'}, 250);
				$(this).attr('id', 'footer-off-home-up');
			} else {
				$(this).stop().animate({bottom: '0'}, 250);
				$(this).attr('id', 'footer-off-home');			
			}
		}	
	);
	
	// Hover shtuff
	$('#footer-off-home').hover(
		// Mouse over
		function() {
			$(this).find('#footer-off-home-over').stop().animate({opacity: 1}, 250 );
		},
		
		// Mouse out
		function() {	
			$(this).find('#footer-off-home-over').stop().animate({opacity: 0}, 250 );
		} 
	
	);
	
	// Featured Menu Footer-shmooter
	// Clicky shtuff
	$('#footer-off-home-featured').click(
		function() {
		
			var id = $(this).attr('id');
		
			if (id == 'footer-off-home-featured') {
				$(this).stop().animate({bottom: '256px'}, 250);
				$(this).attr('id', 'footer-off-home-featured-up');
			} else {
				$(this).stop().animate({bottom: '0'}, 250);
				$(this).attr('id', 'footer-off-home-featured');			
			}
		}	
	);
	
	// Hover shtuff
	$('#footer-off-home-featured').hover(
		// Mouse over
		function() {
			$(this).find('#footer-off-home-over').stop().animate({opacity: 1}, 250 );
		},
	...
	//Credit: Michael Barret - net.tutsplus.com - http://codecanyon.net/user/mike182uk?ref=NetPremium
